...
def prompt_input(pkginfo, prompt):
    ...
    with tempfile.TemporaryDirectory(dir=dpath) as d:
        ...
        header = (
            ...
            "Webpage: {}/packages.php?ID={!s}".format(
                AUR.common.AUR_URL, pkginfo["ID"]
            ),
...
class Aurtomatic(object):
    ...
    def __init__(self, cookiejar_path=None, cookiejar=None, token=None):
        """
    ...
    def submit_package_form(
        ...
        if comment is not None:
            XCGF.warn_deprecated(
                'keyword argument "comment" in submit_package_form is deprecated in favor of "value"'
            )
            value = comment
        ...
        if action in PACKAGE_ACTIONS:
            ...
            if value is None:
                value = prompt_input(
                    pkginfo, "Enter keywords (or nothing to clear them)."
                )
            if value:
    ...
    def upload_pkg(self, fpath, confirm=True):
        ...
        """
        raise AurtomaticError(
            "Package uploads are no longer supported since AUR 4.0 due to the move to Git repos via SSH. Handling that is better left to user scripts."
        )
...
def parse_args(args=None):
    ...
    parser.add_argument(
        "-v", "--vote", action="store_true", help="Vote for each uploaded package."
    )
...
def main(args=None):
        ...
        for path in pargs.paths:
            if os.path.isdir(path):
                ...
                if ps:
                    ...
                    raise AurtomaticError(
                        "no source package found in directory ({})".format(path)
                    )
            else:
                pkgs.append(path)
    ...
    with CookieWrapper(
        path=pargs.cookiejar, action=action, login_file=pargs.login
    ) as aurtomatic:
        for pkg in pkgs:
